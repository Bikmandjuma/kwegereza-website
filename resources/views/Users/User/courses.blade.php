...
<div class="p-4 md:p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold" style="color:#094939">Amasomo Agenda Nyiyandikishijemo</h1>
        <a href="{{ route('guest.courses') }}" class="px-4 py-2 text-sm font-semibold rounded-xl" style="background:#058e48;color:#fff">
        ...
        <div class="grid gap-5 md:grid-cols-2">
            @foreach($enrollments as $enrollment)
                ...
                <div class="p-5 bg-white shadow-lg rounded-3xl">
                    <h2 class="mb-1 font-bold" style="color:#094939">{{ $enrollment->course->title }}</h2>
                    ...
                    <div class="h-2 mb-4 overflow-hidden bg-gray-100 rounded-full">
                        <div class="h-full" style="width:{{ $percent }}%;background:#058e48"></div>
                    </div>
                    <a href="{{ route('guest.course.show', $enrollment->course->slug) }}" class="text-sm font-semibold" style="color:#058e48">Komeza →</a>
                </div>
            @endforeach
        </div>
    @endif
