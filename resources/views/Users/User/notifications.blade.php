...
<div class="p-4 md:p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold" style="color:#094939">Ibimenyetso (Notifications)</h1>

        @if($notifications->where('read_at', null)->count())
        <form action="{{ route('student.notifications.readAll') }}" method="POST">
            @csrf
            <button class="px-4 py-2 text-sm font-semibold rounded-xl" style="background:#e2b45f;color:#094939">
    ...
    <div class="space-y-3">
        ...
        <a href="{{ $n->data['url'] ?? '#' }}"
           ...
           class="flex items-start w-full gap-3 p-4 bg-white shadow cursor-pointer rounded-2xl {{ $n->read_at ? 'opacity-60' : '' }}">
            <span class="mt-1 w-2.5 h-2.5 rounded-full flex-shrink-0" style="background: {{ $n->read_at ? '#ccc' : '#058e48' }}"></span>
            <div>
                <p class="font-semibold" style="color:#094939">{{ $n->data['title'] ?? 'Itangazo' }}</p>
