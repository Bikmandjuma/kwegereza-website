...
<div class="p-4 md:p-6">
    ...
    <div class="flex items-center gap-4 p-6 mb-6 text-white shadow-lg bg-gradient-to-br from-[#e2b45f] to-[#c8a36c] rounded-3xl">
    ...
    <h2 class="mb-4 text-lg font-bold" style="color:#094939">{{ __('badges.earned_heading') }}</h2>
    ...
    <div class="grid gap-4 mb-8 md:grid-cols-3">
        @forelse($allBadges as $badge)
            ...
            <div class="p-5 text-center bg-white shadow rounded-2xl {{ $isEarned ? '' : 'opacity-40 grayscale' }}">
                ...
                <p class="font-bold" style="color:#094939">{{ $badge->name }}</p>
                <p class="mt-1 text-xs text-gray-400">{{ $badge->description }}</p>
                @if($isEarned)
                    <span class="inline-block px-2 py-0.5 mt-2 text-[10px] font-bold text-white rounded-full" style="background:#058e48">{{ __('badges.earned_tag') }}</span>
                @endif
            </div>
        @empty
