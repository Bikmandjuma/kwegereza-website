...
<div class="max-w-2xl p-4 mx-auto md:p-6">
    ...
    <div class="p-8 mb-6 text-center bg-white shadow-lg rounded-3xl">
        <div class="flex items-center justify-center w-20 h-20 mx-auto mb-4 text-3xl font-bold text-white rounded-full"
             style="background: {{ $attempt->passed ? '#058e48' : '#e11d48' }}">
            {{ $attempt->percentage }}%
        </div>
        <h1 class="mb-1 text-xl font-bold" style="color:#094939">{{ $attempt->quiz->title }}</h1>
    ...
    <a href="{{ route('student.quizzes.history') }}" class="block mt-6 text-sm font-semibold text-center" style="color:#058e48">
