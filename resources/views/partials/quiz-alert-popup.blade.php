<div id="kiuQuizAlertModal" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center; padding:16px; background:rgba(9,73,57,.55);">
  <div style="width:100%; max-width:380px; background:#fff; border-radius:24px; box-shadow:0 20px 60px rgba(0,0,0,.3); overflow:hidden;">
    ...
    <div style="padding:20px 24px;">
      ...
      <button style="width:100%; padding:12px; border:none; border-radius:16px; background:#e2b45f; color:#fff; font-weight:700; font-size:13px; cursor:pointer;">
        Nahamukanya In shaa Allah, Jazakallahu khayran
      </button>
      <div style="display: flex; justify-content: center;">
          <button onclick="kiuDismissQuizAlert()" style="color: red;font-size: 15px;margin-top: 10px;">
              Close - X
          </button>
      </div>
    </div>
...
(function() {
  ...
  function kiuPlayQuizAlertSound() {
    try {
      ...
      // A simple, pleasant two-note chime (like a gentle "ding-dong"),
      // built with the Web Audio API rather than an external sound file —
      // no asset to host, and no attempt to imitate any specific
      // platform's actual proprietary notification sound.
      ...
      // page at least once — that's expected and not worth erroring over,
      // the visual popup still shows either way.
